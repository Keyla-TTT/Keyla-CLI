...
suspend fun configMode(
    ...
    coroutineScope {
        session {
            ...
            fun updateState(newState: ConfigState) { state = newState }
            fun setScreen(newScreen: ConfigScreen) { screen = newScreen }
            ...
            section {
                ...
                when (screen) {
                    is ConfigScreen.MainMenu -> {
                        ...
                        options.forEachIndexed { i, opt ->
                            if (i == configMenuIndex) cyan { textLine("> $opt") }
                            else textLine("  $opt")
                        }
                        textLine()
                ...
                onKeyPressed {
                    when (screen) {
                        is ConfigScreen.MainMenu -> {
                            when (key.name) {
                                "UP" -> { configMenuIndex = (configMenuIndex - 1).coerceAtLeast(0) }
                                "DOWN" -> { configMenuIndex = (configMenuIndex + 1).coerceAtMost(2) }
                                "ENTER" -> {
                        ...
                        is ConfigScreen.UpdateConfig -> {
                            when (key.name) {
                                ...
                                "BACKSPACE" -> when (updateConfigField) {
